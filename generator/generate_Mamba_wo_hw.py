...
stage_outer = math.ceil(math.log2(seq) / math.log2(r))
stage_inner = int(math.log2(r))
...
for i in range(stage_outer):
    for j in range(stage_inner):
        kernel = dataflow_graph.kernels.add()
        kernel.name = 'Scan_stage_'+str(i*stage_inner+j)
        kernel.id = cnt
        kernel.fwd_bwd = 1
        kernel.type = 1
        kernel.config = -1
...
for i in range(4, 4+stage_outer*stage_inner):
    connection = dataflow_graph.connections.add()
    connection.id = cnt
...
connection.endIdx = 3+stage_outer*stage_inner+1
cnt += 1

connection = dataflow_graph.connections.add()
connection.id = cnt
connection.startIdx = 3+stage_outer*stage_inner+1
connection.endIdx = 3+stage_outer*stage_inner+2




# write to text file
with open('generator/Mamba_wo_hw.txt', "w") as file:
